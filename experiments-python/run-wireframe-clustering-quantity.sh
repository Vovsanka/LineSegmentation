...
wireframe_out_dir="${experiments_dir}/wireframe-results-quantity"
#
...
for img_path in "${wireframe_src_dir}/test"/*.jpg; do
    ...
    # 
    "$executable" "$img_path" "$working_state_dir" "--st" "--th" "--on-cl" "--on-el" "--mws"
    "$executable" "$img_path" "$working_state_dir" "--st" "--it" "--on-cl" "--on-el" "--mws"
    "$executable" "$img_path" "$working_state_dir" "--bm" "--th" "--on-cl" "--on-el" "--mws"
